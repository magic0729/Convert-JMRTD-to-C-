...
{
	public sealed class DESedeSecureMessagingWrapper : SecureMessagingWrapper
	{
		public DESedeSecureMessagingWrapper(SecretKey ksEnc, SecretKey ksMac, int maxTranceiveLength, bool shouldCheckMAC, long ssc)
			: base(ksEnc, ksMac, maxTranceiveLength, shouldCheckMAC, ssc) { }

		public DESedeSecureMessagingWrapper(DESedeSecureMessagingWrapper other)
		...
		public override CommandAPDU Wrap(CommandAPDU command)
		{
			...
			byte[] do8e = TLV.WrapDO(0x8E, AESCMAC.Compute(new SecretKey(GetMACKey().GetEncoded(), "AES").GetEncoded(), mInput, 8));
		...
		public override ResponseAPDU Unwrap(ResponseAPDU response)
		{
			...
			byte[] mac = do8e;
			byte[] expect = AESCMAC.Compute(new SecretKey(GetMACKey().GetEncoded(), "AES").GetEncoded(), mInput, 8);
			int diff = 0; for (int i = 0; i < mac.Length && i < expect.Length; i++) diff |= mac[i] ^ expect[i];
		...
		public override object Type => "DESede";
